...
def main() -> int:
    ...
    print()
    print("  DoH rather than `dig`, on purpose. A local resolver that redirects")
    print("  outbound port 53 — a Pi-hole, say — answers `dig @1.1.1.1` itself,")
    print("  so naming an upstream does not bypass its cache. And it will have")
    print("  cached the NEGATIVE answer from any query made before this record")
    print("  existed, for up to the zone's SOA minimum: 1800s on Cloudflare.")
    print("  That looks exactly like the write having failed. DoH leaves port 53")
    print("  alone and asks the resolver that actually matters here.")
